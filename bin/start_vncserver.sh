...
clientOS="$4"
geometry="$5"
if [[ "${6}x" != x ]] ; then DISPLAYPORT=":${6}" ; fi 
...
/usr/bin/vncserver ${DISPLAYPORT} ${geoarg} -localhost -verbose -name "$USER at `hostname -s` VNC2HPC v$vnc2hpc_version $windowmanager `date`" -autokill -depth 16 -xstartup "$HOME/.vnc2hpc/xstartup" &>$LOG
